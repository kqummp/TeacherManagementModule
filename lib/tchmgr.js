...
tchmgr.Cancel = async function (reserve_id, cancel_reason, session_user, uid) {
  if (typeof cancel_reason === "undefined" || cancel_reason === "") {
    throw new Error(message.invalid_field);
  }

  if (!(filter.filter(cancel_reason))) {
    throw new Error(message.invalid_field);
  }
  ...
  if (result.message === message.success) {
    if (result.status === "canceled") {
      throw new Error(message.canceled);
    } else {
      let oid = kqudie.String2ObjectId(reserve_id);
      let update_query = {
        "_id": oid
...
tchmgr.Reject = async function (reserve_id, reject_reason, session_user, uid) {
  if (typeof reject_reason === "undefined" || reject_reason === "") {
    throw new Error(message.invalid_field);
  }

  if (!(filter.filter(reject_reason))) {
    throw new Error(message.invalid_field);
  }
