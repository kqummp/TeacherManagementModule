...
tchmgr.QueryByReserveId = async function (reserve_id, session_user) {
  ...
  if (result.length === 0) {
    throw new Error(message.oid_error);
  } else {
    let return_data = {
      "message": message.success,
      "data": result[0]
...
tchmgr.Status = async function (reserve_id, session_user) {
...
tchmgr.Accept = async function (reserve_id, session_user) {
  let result;
  try {
    result = await tchmgr.Status(reserve_id, session_user);
  } catch (err) {
    throw err;
  }
...
tchmgr.Reject = async function (reserve_id, session_user) {
  let result;
  try {
    result = await tchmgr.Status(reserve_id, session_user);
  } catch (err) {
  ...
  if (result.message === message.success) {
    if (result.status === "accepted") {
      ...
      let raw_reservation;
      try {
        raw_reservation = await tchmgr.QueryByReserveId(reserve_id, session_user);
      } catch (err) {
        throw err;
      }
...
tchmgr.Arrange = async function (session_user, data) {
  if (typeof session_user === "undefined") {
    throw new Error(message.no_login);
  }
