...
describe('cancelTest', function () {
    ...
    it('acceptTest#1', async function () {
      ...
      try {
        result = await tchmgr.Cancel(oid, "asdasd", 1000000, 1000000);
      } catch (err) {
        catch_err = err;
      }
    ...
    it('acceptTest#2', async function () {
      ...
      let oid = "5bb38ef916f47987b7fe0000";
      try {
        result = await tchmgr.Cancel(oid, "asdasd", 1000000, 1000001);
      } catch (err) {
        catch_err = err;
      }
      expect(result).to.be.an("undefined");
    ...
    it('acceptTest#4', async function () {
      ...
      let oid = "5bb38ef916f47987b7fe0000";
      try {
        result = await tchmgr.Cancel(oid, "", 1000000, "1000000");
      } catch (err) {
        catch_err = err;
      }
      expect(result).to.be.an("undefined");
    ...
    it('acceptTest#5', async function () {
      ...
      let oid = "5bb38ef916f47987b7fe0000";
      try {
        result = await tchmgr.Cancel(oid, "{$ne: 1}", 1000000, "1000000");
      } catch (err) {
        catch_err = err;
      }
      expect(result).to.be.an("undefined");
    ...
    it('acceptTest#6', async function () {
      ...
      expect(detail_result[0].status).to.be.equal("canceled");

    });

    it('acceptTest#7', async function () {
      let result, catch_err;
      let oid = ins_id;
      try {
        result = await tchmgr.Cancel(oid, "asdasd", 1000000, 1000000);
      ...
      expect(result).to.be.an("undefined");
      expect(catch_err).to.be.an("Error");
      expect(catch_err.message).to.be.equal(message.canceled);
    });
